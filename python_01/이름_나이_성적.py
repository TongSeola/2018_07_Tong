...
class ScoreDB(QWidget):
    def __init__(self):
        ...
        self.initUI()
        self.dbfilename = 'name_age_old.dat'
    ...
    def addAction(self):
        ...
        if agetext.isnumeric() and float(scoretext):
            record = {'Name': nametext, 'Age': int(agetext), 'Score': float(scoretext)}
            self.scoredb += [record]
            self.showScoreDB()
        elif agetext.isnumeric()==False and float(scoretext):
            QMessageBox.question(self, 'title', "Age에 숫자를 입력해주세요", QMessageBox.Yes)
        elif agetext.isnumeric() and float(scoretext)==False :
