...
class HangmanGame(QWidget):
    ...
    def guessClicked(self):
        ...
        if len(guessedChar) != 1:
            self.message.setText("Please enter one alphabet")
            return

        # 이미 사용한 글자인지를 판단하고, 아닌 경우 메시지 출력, 리턴
        if guessedChar in self.guess.guessedChars :
            self.message.setText("This alphabet is already used")
            return
        ...
        if success == False:
            ...
            # 메시지 출력
            self.message.setText("Game is not over")
        ...
        if self.guess.finished():
            ...
            self.message.setText("Fail, the world is "+ self.word.randFromDB())
            self.gameOver = True
